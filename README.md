...
## ***开发成果
> 这里是bilibili演示视频地址
---https://www.bilibili.com/video/BV1oV4y1H7Jn/?vd_source=6ad92130426c91fc41f1386e853116bc
...
- 功能2：实现图像的上下反转与左右反转，并以此拓展，实现对不同角度的定量旋转
    定义布尔类型的code并作为翻转函数的参数，true为上下翻转，false为左右翻转。
        上下翻转就是对图片以水平中心线为对称轴进行像素点的互换，以高度（height）
    的二分之一为分界线，调用chg函数。
        左右翻转与上下翻转同理，以宽度（width）的二分之一为分界线，调用swap函数。
    代码如下：
...
##使用说明：
请在qtcreator运行该程序，并保证有gcc/clang集成编译环境下运行，本程序开发是采用QT 5.152版本，并使用mingw。
## 引用参考
